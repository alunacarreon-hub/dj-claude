...
// 2. Buscar cada seed en Spotify - primero búsqueda estricta, luego flexible
const found=[];
const foundUris=new Set();

function buildTrack(tk){
  return {uri:tk.uri,title:tk.name,artist:tk.artists.map(a=>a.name).join(', '),duration:msToTime(tk.duration_ms),art:tk.album&&tk.album.images&&(tk.album.images[1]||tk.album.images[0])?(tk.album.images[1]||tk.album.images[0]).url:null};
}

// Primera pasada: búsqueda estricta con filtros track:/artist:
...
if(!foundUris.has(tk.uri)){foundUris.add(tk.uri);found.push(buildTrack(tk));}
}
...
function renderPlaylist(){const el=document.getElementById('playlist');el.innerHTML='';document.getElementById('playlist-count').innerHTML='Playlist &middot; '+currentTracks.length+' canciones';currentTracks.forEach((t,i)=>{const item=document.createElement('div');item.className='playlist-item'+(i===currentIdx?' active':'');item.innerHTML='<div class="playlist-num">'+(i===currentIdx&&isPlaying?'&#9834;':i+1)+'</div><div class="playlist-info"><div class="playlist-title">'+t.title+'</div><div class="playlist-artist">'+t.artist+'</div></div><div class="playlist-dur">'+t.duration+'</div>';item.querySelector('.playlist-info').onclick=()=>jumpToTrack(i);el.appendChild(item);});}
