...
async function spotifySearch(title,artist){if(!accessToken){showAuth();throw new Error("no token");}const q=encodeURIComponent(`${title} ${artist}`);const res=await fetch(`https://api.spotify.com/v1/search?q=${q}&type=track&limit=1`,{headers:{Authorization:`Bearer ${accessToken}`}});const data=await res.json();if(data.error?.status===401){sessionStorage.removeItem("spotify_token");accessToken=null;showAuth();throw new Error("expired");}return data.tracks?.items?.[0]||null;}
