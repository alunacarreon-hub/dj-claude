...
let accessToken=null,player=null,deviceId=null,currentTracks=[],currentIdx=0,isPlaying=false,progressInterval=null,waveInterval=null,exampleInterval=null,exampleIdx=0,recognition=null,likedTracks=new Set(),playlistOpen=false,spotifyPlayer=null;
window.addEventListener('load',()=>{
const hash=new URLSearchParams(window.location.hash.replace('#','?'));
const token=hash.get('access_token');
if(token){accessToken=token;sessionStorage.setItem('spotify_token',token);window.history.replaceState({},'','/');showMain();}
else{const saved=sessionStorage.getItem('spotify_token');if(saved){accessToken=saved;showMain();}else{showAuth();}}
...
function login(){const url=`https://accounts.spotify.com/authorize?client_id=${CLIENT_ID}&response_type=token&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=${encodeURIComponent(SCOPES)}`;window.location.href=url;}
...
function startListening(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert('Usa Chrome para reconocimiento de voz.');return;}
setPhase('listening');recognition=new SR();recognition.lang='es-MX';recognition.continuous=false;recognition.interimResults=false;
recognition.onresult=(e)=>{processPrompt(e.results[0][0].transcript);};
recognition.onerror=(e)=>{setPhase('idle');};
recognition.onend=()=>{if(document.getElementById('phase-listening').classList.contains('active'))setPhase('idle');};
recognition.start();}
function stopListening(){if(recognition){recognition.stop();recognition=null;}}
async function callClaude(prompt){
const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:'Eres un DJ experto. Devuelve SOLO JSON válido sin texto extra ni markdown con este formato: {"playlist_name":"nombre","tracks":[{"title":"canción","artist":"artista"}]}. Entre 8 y 15 canciones reales. Para artistas específicos elige sus mayores éxitos.',messages:[{role:'user',content:prompt}]})});
...
}catch(err){console.error(err);if(String(err).includes('401')){sessionStorage.removeItem('spotify_token');alert('Sesión expirada. Vuelve a conectar.');showAuth();}else{alert('Algo falló. Intenta de nuevo.');setPhase('idle');}}}
...
function renderPlaylist(){const el=document.getElementById('playlist');el.innerHTML='';document.getElementById('playlist-count').textContent=`Playlist · ${currentTracks.length} canciones`;currentTracks.forEach((t,i)=>{const item=document.createElement('div');item.className='playlist-item'+(i===currentIdx?' active':'');item.innerHTML=`<div class="playlist-num">${i===currentIdx&&isPlaying?'♪':i+1}</div><div class="playlist-info"><div class="playlist-title">${t.title}</div><div class="playlist-artist">${t.artist}</div></div><div class="playlist-dur">${t.duration}</div><button style="background:none;border:none;cursor:pointer;font-size:14px" data-i="${i}">${likedTracks.has(i)?'💚':'🤍'}</button>`;item.querySelector('.playlist-info').onclick=()=>jumpToTrack(i);item.querySelector('button').onclick=(e)=>{e.stopPropagation();toggleLike(i);};el.appendChild(item);});}
...
function toggleLike(idx){likedTracks.has(idx)?likedTracks.delete(idx):likedTracks.add(idx);renderPlaylist();document.getElementById('btn-like').textContent=likedTracks.has(currentIdx)?'💚':'🤍';}
...
document.getElementById('btn-new-prompt').addEventListener('click',()=>{clearInterval(progressInterval);likedTracks.clear();currentTracks=[];setPhase('idle');});}
