...
async function handleCallback(){const params=new URLSearchParams(window.location.search);const code=params.get('code');if(!code)return false;const verifier=localStorage.getItem('pkce_verifier');const res=await fetch('/api/token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,code_verifier:verifier,redirect_uri:REDIRECT_URI})});const data=await res.json();if(data.access_token){accessToken=data.access_token;localStorage.setItem('spotify_token',data.access_token);window.history.replaceState({},'','/');return true;}console.error('Token error:',data);return false;}
...
async function callClaude(prompt){const res=await fetch('/api/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:'Eres un DJ experto. Devuelve SOLO JSON válido sin texto extra ni markdown con este formato exacto: {"playlist_name":"nombre","tracks":[{"title":"cancion","artist":"artista"}]}. Entre 20 y 25 canciones reales, variadas y conocidas. Incluye éxitos principales, canciones populares y algunas joyas menos conocidas.',messages:[{role:'user',content:prompt}]})});const data=await res.json();return JSON.parse(data.content[0].text.replace(/```json|```/g,'').trim());}
async function spotifySearch(title,artist){if(!accessToken){showAuth();throw new Error("no token");}const q=encodeURIComponent(`${title} ${artist}`);const res=await fetch(`https://api.spotify.com/v1/search?q=${q}&type=track&limit=1`,{headers:{Authorization:`Bearer ${accessToken}`}});const data=await res.json();if(data.error?.status===401){localStorage.removeItem("spotify_token");accessToken=null;showAuth();throw new Error("expired");}return data.tracks?.items?.[0]||null;}
...
}catch(err){console.error(err);if(String(err).includes('401')){localStorage.removeItem('spotify_token');alert('Sesión expirada.');showAuth();}else{alert('Algo falló. Intenta de nuevo.');setPhase('idle');}}}
